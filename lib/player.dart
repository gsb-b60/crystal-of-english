...
class Player extends SpriteComponent with HasGameRef<MyGame>, CollisionCallbacks {
  ...
  Future<void> onLoad() async {
    ...
    sprite = Sprite(spriteSheet, srcPosition: Vector2(0, frameSize.y), srcSize: frameSize);
  ...
  void update(double dt) {
    ...
    if (joystick!.direction != JoystickDirection.idle) {
      if (moveLeft && (!collided || collisionDirection == JoystickDirection.right)) {
        velocity.x = -speed;
        currentRow = 0;
        facingLeft = true;
      } else if (moveRight && (!collided || collisionDirection == JoystickDirection.left)) {
        velocity.x = speed;
        currentRow = 0;
        facingLeft = false;
      } else if (moveUp && (!collided || collisionDirection == JoystickDirection.down)) {
        velocity.y = -speed;
        currentRow = 2;
      } else if (moveDown && (!collided || collisionDirection == JoystickDirection.up)) {
        velocity.y = speed;
        currentRow = 1;
      }
    }
  ...
  void onCollisionStart(Set<Vector2> intersectionPoints, PositionComponent other) {
    ...
    if (other is HitboxComponent) {
      print('Collision detected with HitboxComponent at $intersectionPoints, player position: $position');
      if (!collided) {
        collided = true;
  ...
  void onCollisionEnd(PositionComponent other) {
    ...
    if (other is HitboxComponent) {
      print('Collision ended with HitboxComponent at player position: $position');
      collided = false;
